...
public class PagerTag extends SimpleTagSupport{
	...
	public void doTag() throws JspException,IOException {
		...
		//判断总记录条数
		if(recordCount > 0) {
			//需要显示分页标签，计算出总数 需要多少页
			totalPage = (this.recordCount - 1) / this.pageSize + 1;
			//判断上一页或下一页需不需要加a标签
			if(this.pageIndex == 1) {
				str.append("<spend class = 'disabled'>上一页</span>");
				//计算中间的页码
				this.calcPage(str);
				//下一页需不需要a标签
			...
			res.append("<tr><td  style='COLOR:  #0061e;  MARGIN-RIGHT:  3px;PADDING-TOP: 2px; TEXT-DECORATION: none'>"+str.toString());
